...
def build_grouped_prompt(samples):
    ...
    return (
        ...
        "The input consists of a sequence of key frames from one video.\n"
        "Answer every multiple-choice question for this same video.\n"
        "Return only a JSON object whose keys are question_id and whose values are a single option letter like A, B, C, D, E, or F.\n\n"
        + "\n\n".join(blocks)
    )
...
def parse_response(text, samples):
    ...
    try:
        ...
        if start != -1 and end != -1 and end > start:
            ...
            if isinstance(parsed, dict) and "question_id" in parsed and "answer" in parsed:
                qid = str(parsed["question_id"]).strip()
    ...
    result = {}
    for sample in samples:
        pattern = re.compile(rf"{re.escape(sample['question_id'])}\s*[:=]\s*([A-F])", re.I)
...
def infer_video(api_base, api_key, model_name, frames, samples, max_tokens, temperature):
...
def process_video(video_dir, video_name, samples, args):
    ...
    if not os.path.exists(video_path):
        return video_name, [], f"missing video: {video_path}"

    frames, _ = load_video(video_path, keyframe=None, num_frms=args.num_frames)
    if not frames:
        return video_name, [], f"empty frames for video: {video_name}"

    raw_text, answers = infer_video(
...
def main(args):
    ...
    lock = threading.Lock()
    progress = tqdm(total=len(video_items))
    ...
    with open(output_path, "a", encoding="utf-8") as out_f, open(
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            futures = {
                executor.submit(process_video, args.video_dir, video_name, samples, args): video_name
