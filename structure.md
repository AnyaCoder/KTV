...
- `.codex/skills/`: repository-local Codex skills, including the reusable `codex-guidelines` skill derived from `CODEX.md`.
...
- `run_eval_swarm_openai.py`: OpenAI-compatible VLM frame selection evaluator with sliding-window selection, segment-tree hierarchical summarization, optional evidence-plan routing, and frontier-payload ablations before final VLM QA.
...
- `scripts/`: small utility scripts for dataset preparation, download/extraction, and benchmark preprocessing.
